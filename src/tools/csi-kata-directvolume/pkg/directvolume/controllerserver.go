...
func (dv *directVolume) CreateVolume(ctx context.Context, req *csi.CreateVolumeRequest) (resp *csi.CreateVolumeResponse, finalErr error) {
	...
	for key, value := range req.GetParameters() {
		...
		case utils.KataContainersDirectVolumeType:
			if value == utils.DirectVolumeTypeName || value == "spdkvol" {
	...
	var err error
	if volumeCtx[utils.KataContainersDirectVolumeType] == "spdkvol" {
		volName := req.GetName()
		backingFile := fmt.Sprintf("/var/lib/spdk/rawdisks/%s.raw", volName)

		if err := os.MkdirAll("/var/lib/spdk/rawdisks", 0750); err != nil {
			return nil, status.Errorf(codes.Internal, "failed to create dir /var/lib/spdk/rawdisks: %v", err)
		}
